...
describe('<SignUp />', () => {
  ...
  it('renders the sign up page with a form submission and signs a user up', async () => {
    ...
    await act(async () => {
      checkUsernameIsExists.mockImplementation(() => Promise.resolve(true)); // as true but inverse in the code
      ...
      await fireEvent.change(getByPlaceholderText('Password confirmation'), { target: { value: 'password' } });
      fireEvent.submit(getByTestId('sign-up'));

      expect(document.title).toEqual('Sign Up - Instagram');
      await expect(checkUsernameIsExists).toHaveBeenCalled();
      await expect(checkUsernameIsExists).toHaveBeenCalledWith('karl');
      ...
      await waitFor(() => {
        ...
        expect(getByPlaceholderText('Password confirmation').value).toBe('password');
  ...
  it('renders the sign up page but an error is present (username exists)', async () => {
    ...
    await act(async () => {
      checkUsernameIsExists.mockImplementation(() => Promise.resolve([false])); // as true but inverse in the code
      ...
      await waitFor(() => {
        ...
        expect(getByPlaceholderText('Password confirmation').value).toBe('');
